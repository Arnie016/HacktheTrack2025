...
def detect_stints(
    ...
    min_stint_laps: int = 3,  # Minimum laps for valid stint (filters out artifacts)
) -> list[Stint]:
    """
    ...
    Args:
        ...
        min_stint_laps: Minimum laps for a valid stint (default 3, filters artifacts)
    
    Returns:
        List of Stint objects (only valid stints >= min_stint_laps)
    """
    vehicle_laps = laps_df[laps_df["vehicle_id"] == vehicle_id].copy()
    
    # FILTER OUT SENTINEL VALUES (lap 32768, lap 0, invalid lap numbers)
    vehicle_laps = vehicle_laps[
        (vehicle_laps["lap"] > 0) & 
        (vehicle_laps["lap"] < 1000)  # Reasonable max lap for any race
    ].copy()
    ...
    for i in range(1, len(vehicle_laps)):
        ...
        if time_diff > pit_threshold_ms:
            ...
            if len(stint_laps) > 0:
                ...
                # Skip stints with invalid lap times, lap numbers, OR TOO SHORT
                if (best_lap_time_ms > 0 and 
                    current_stint_start >= 1 and 
                    end_lap >= 1 and
                    stint_length >= min_stint_laps):  # FILTER ARTIFACTS
                    stints.append(Stint(
                        car_id=vehicle_id,
    ...
    if len(final_laps) > 0:
        final_stint_length = len(final_laps)
        ...
        if (best_lap_time_ms > 0 and 
            current_stint_start >= 1 and 
            end_lap_final >= 1 and
            final_stint_length >= min_stint_laps):  # FILTER ARTIFACTS
            stints.append(Stint(
                car_id=vehicle_id,
                start_lap=int(current_stint_start),
